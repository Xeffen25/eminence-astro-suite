...
import OpenGraphSource from "@package/components/OpenGraph.astro?raw";
...
| `image`           | `ImageMetadata \| ComponentProps<typeof OpenGraphImage>` | -                                                     | No       | Imported Astro image or structured image metadata rendered as `og:image` tags.                                        |
...
Pass an imported local image directly. Its generated URL, width, height, and format are used for the related `og:image:*` tags automatically.
...
Use the structured form when you also need fields such as `alt`, `secureUrl`, or explicit overrides:

```astro
<OpenGraph title="Home" image={{ src: socialImage, alt: "Home page" }} />
```
...
### Explicit type prop takes precedence
...
<Code code={OpenGraphSource} lang="astro" />
