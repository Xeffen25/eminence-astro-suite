...
type ResolvedTagConfig = {
  ...
  icons: IconTag[]; // detected public icons merged with headTags.icons
...
Server-only options (like `capojs`, `robotsTxt`, `securityTxt`, and `sitemap`) are **never** included in the virtual module and cannot be accessed by components.
...
1. Extracts only the `headTags` field
