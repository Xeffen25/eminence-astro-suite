...
description: Internal virtual module that distributes head tag configuration to components at build time. Manages config serialization and module resolution.
head:
  - tag: title
...
<Aside>
  This pattern is only relevant if you are extending or authoring your own head
  tag components. Regular users should configure defaults through the
  integration `head` option and use components as normal.
</Aside>
