...
The `eminence` integration is configured through a single options object. Each key maps to file generation, public icon discovery, or global head tag defaults. Features can be disabled by passing `false`. Omitting a feature key logs a recommendation warning where applicable.
