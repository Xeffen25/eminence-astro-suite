...
type NameOrShortName = { name: string; short_name?: string } | { short_name: string; name?: never };
...
export type WebManifestOptions = NameOrShortName & DisplayOrDisplayOverride & WebManifestBase;
...
export async function generateManifest({ dir, options, logger }: IntegrationRuntimeContext): Promise<void> {
	...
	if (input === false) {
		if (outputExists) {
			...
			logger.info(`No "${WEB_MANIFEST_RELATIVE_PATH}" file exists and no file was generated.`);
		}
	...
	if (typeof input !== "object" || input === null) {
		logger.error("Invalid manifest configuration: expected an object with required PWA fields.");
		throw new Error("Invalid manifest configuration: expected an object with required PWA fields.");
	}
	...
	try {
		...
		logger.error(`Failed to generate "${WEB_MANIFEST_RELATIVE_PATH}": ${message}`);
		throw error;
	}
}
