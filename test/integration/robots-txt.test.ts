...
describe("Integration - RobotsTxt", () => {
	...
	it("writes robots.txt with directives and sitemap entries", async () => {
		...
		expect(logger.info).toHaveBeenCalledWith(`Generated "${ROBOTS_TXT_RELATIVE_PATH}"`);
	});
	...
	it("logs an error before throwing when a directive path is invalid", async () => {
		...
		).rejects.toThrow('Invalid disallow value "private": expected a path starting with "/".');
	...
	it("throws when crawlDelay is negative", async () => {
		...
		).rejects.toThrow('Invalid crawlDelay value "-1": expected a non-negative number.');
	});
	...
	it("throws when rules are missing or empty", async () => {
		...
		).rejects.toThrow("Invalid robotsTxt configuration: expected at least one rule.");
	});
});
