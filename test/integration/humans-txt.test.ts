...
import {
  HUMANS_TXT_RECOMMENDATION,
  validateHumansTxtInBuildOutput,
} from "@package/integration/humans-txt";
...
describe("Integration - HumansTxt", () => {
  ...
  const createContext = (
    humansTxt: boolean | undefined,
  ): IntegrationRuntimeContext => ({
    config: { outDir: outDirUrl } as AstroConfig,
    dir: outDirUrl,
    options: { headTags: { humansTxt } },
  ...
  it("does not warn when humansTxt is true and humans.txt exists", async () => {
    await writeFile(
      join(outputDir, "humans.txt"),
      "Team humans.txt content",
      "utf-8",
    );
